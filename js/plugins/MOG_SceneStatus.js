...
/*:
 * @plugindesc (v1.0)[v1.3]  主菜单 - 全自定义状态界面
 ...
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 先确保项目img文件夹下是否有menus_status文件夹！（img/menus_status）
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneStatus');  
	...
	Moghunter.scStatus_WeaponX = Number(Moghunter.parameters['平移-装备槽1 X'] || 7);
	Moghunter.scStatus_WeaponY = Number(Moghunter.parameters['平移-装备槽1 Y'] || 347);
	Moghunter.scStatus_ShieldX = Number(Moghunter.parameters['平移-装备槽2 X'] || 7);
	Moghunter.scStatus_ShieldY = Number(Moghunter.parameters['平移-装备槽2 Y'] || 398);	
	Moghunter.scStatus_HelmetX = Number(Moghunter.parameters['平移-装备槽3 X'] || 7);
	Moghunter.scStatus_HelmetY = Number(Moghunter.parameters['平移-装备槽3 Y'] || 449);	
	Moghunter.scStatus_ArmorX = Number(Moghunter.parameters['平移-装备槽4 X'] || 261);
	Moghunter.scStatus_ArmorY = Number(Moghunter.parameters['平移-装备槽4 Y'] || 376);	
	Moghunter.scStatus_AccX = Number(Moghunter.parameters['平移-装备槽5 X'] || 261);
	Moghunter.scStatus_AccY = Number(Moghunter.parameters['平移-装备槽5 Y'] || 427);
...
ImageManager.loadMenusstatus = function(filename) {
    return this.loadBitmap('img/menus_status/', filename, 0, true);
};
...
Scene_Status.prototype.refreshBust = function() {
     this._bust.bitmap = ImageManager.loadPicture(Moghunter.menu_status_bust_list[this._actor._actorId]);
