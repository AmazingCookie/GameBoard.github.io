...
/*:
 * @plugindesc (v1.0)[v1.2]  主菜单 - 全自定义存档界面
 ...
 * =============================================================================
 * 可完全自己定义的存档界面设置。
 * 该配置包含选择窗口、角色窗口、游戏截图、杂项数据四个部分。
 * 参数如何配的详细介绍去看看插件详细说明"窗口与布局.docx"。
 * 【现已支持插件关联资源的打包、加密】
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 先确保项目img文件夹下是否有menus_save文件夹！   (img/menus_save)
 * 先确保项目img文件夹下是否有pictures文件夹！     (img/pictures)
 * 如果没有，需要自己建立。需要配置资源文件：
 *
 * 资源-整体布局    (menus_save文件夹)
 * 资源-角色窗口    (menus_save文件夹)
 * 资源-选择指针    (menus_save文件夹)
 * 资源-可选对象    (menus_save文件夹)
 *
...
ImageManager.loadMenusFile = function(filename) {
    return this.loadBitmap('img/menus_save/', filename, 0, true);
};
...
Scene_File.prototype.refreshActorPicture = function() {
	  ...
	  this._ActorPicture.bitmap = ImageManager.loadPicture(Moghunter.menu_save_act_list[this._actorIndex]);
...
if (!Bitmap.prototype.save){
	Bitmap.prototype.toDataURL = function(){	//图片存储大小比较
...
var DM_MSInfo = DataManager.makeSavefileInfo;  //存储图片到save文件中
DataManager.makeSavefileInfo = function(){
...
DataManager.makeSavefileBitmap = function(){ //画图区
...
SceneManager.snapForBackground = function() {
    this._backgroundBitmap = this.snap();　//从地图中获取快照
};