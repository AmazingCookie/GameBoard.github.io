...
/*:
 * @plugindesc (v1.2)[v1.3]  主菜单 - 全自定义主菜单
 ...
 * @min 0
 * @max 255
 * @desc 未选中的其它按钮默认的透明度。0为完全透明，255为完全不透明。
 * @default 160
 *
 * @param 是否使用缩放效果
 * @parent ----按钮----
 ...
 * =============================================================================
 * 可完全自己定义的主菜单设置。
 * 配置分五个部分，角色面板组、按钮组、角色面板、按钮、杂项数据。
 * 更多详细介绍，去看看插件详细手册"关于全自定义主菜单（配置表）.docx"。
 * 【现已支持插件关联资源的打包、加密】
 *
 * -----------------------------------------------------------------------------
 * ----硬性规定
 * 由于这里是主菜单，主菜单要求显示的数据必须健全！
 * 以下数据不可隐藏：
 *   等级数值、角色名、
 *   生命数值、最大生命数值、
 *   魔法数值、最大魔法数值、
 *   状态图标、金钱数值、
 *   游戏时长数值、所在地图名称
 * （当然，如果你一定要隐藏，设置Y坐标1000以上，让玩家看不到就可以了。）
 *
 * -----------------------------------------------------------------------------
 * ----素材规则
 * 这里固定设置了生命条是流动的。
 ...
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 先确保项目img文件夹下是否有menus_button文件夹！（img/menus_button）
 * 先确保项目img文件夹下是否有menus_main文件夹！  （img/menus_main）
 * 先确保项目img文件夹下是否有pictures文件夹！    （img/pictures）
 * 如果没有，需要自己建立。需要配置资源文件：
 *
 * 资源-整体布局（menus_main文件夹）
 * 资源-金钱数值  （menus_main文件夹）
 * 资源-魔法圈    （menus_main文件夹）
 *
 * 资源-角色面板  （menus_main文件夹）
 * 资源-生命条    （menus_main文件夹）
 * 资源-生命数值  （menus_main文件夹）
 * 资源-魔法条    （menus_main文件夹）
 * 资源-魔法数值  （menus_main文件夹）
 * 资源-等级数值  （menus_main文件夹）
 *
 * 角色前视图-1 （pictures文件夹，前视图1与编号为1的角色对应。）
 * 角色前视图-2
 * 角色前视图-3
 * …………
 *
 * 资源-角色头像指针（menus_button文件夹）
 *
 * 角色头像-1       （menus_button文件夹，头像1与编号为1的角色对应。）
 * 角色头像-2
 * 角色头像-3
 * ………
 *
 * 资源-默认按钮    （menus_button文件夹）
 * 资源-道具按钮    （menus_button文件夹）
 * 资源-技能按钮    （menus_button文件夹）
 * 资源-装备按钮    （menus_button文件夹）
 * 资源-状态按钮    （menus_button文件夹）
 * 资源-队形按钮    （menus_button文件夹）
 * 资源-选项按钮    （menus_button文件夹）
 * 资源-保存按钮    （menus_button文件夹）
 * 资源-游戏结束选择按钮（menus_button文件夹）
 * 资源-画廊按钮    （menus_button文件夹）
 * 资源-音乐书按钮  （menus_button文件夹）
 *
 * 自定义按钮-1     （menus_button文件夹）
 * 自定义按钮-2
 * 自定义按钮-3
 * ………
 *
 * 关于自定义菜单中的按钮，需要与你使用的插件名字设置相互匹配。
 ...
 *
 * 如果你实在找不到按钮的关键字，你可以开启 关键字搜索 功能，进入主菜单时，会
 * 弹框显示所有的关键字。
 *
 * -----------------------------------------------------------------------------
 * ----界面与按钮关系表
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneMenu');
	...
	Moghunter.scMenu_HPMeterX = Number(Moghunter.parameters['平移-生命条 X'] || 17);
	Moghunter.scMenu_HPMeterY = Number(Moghunter.parameters['平移-生命条 Y'] || 93);
	Moghunter.scMenu_MPMeterX = Number(Moghunter.parameters['平移-魔法条 X'] || 17);
	Moghunter.scMenu_MPMeterY = Number(Moghunter.parameters['平移-魔法条 Y'] || 144);	
	Moghunter.scMenu_HPNumberX = Number(Moghunter.parameters['平移-生命数值 X'] || 100);
	Moghunter.scMenu_HPNumberY = Number(Moghunter.parameters['平移-生命数值 Y'] || 73);
	Moghunter.scMenu_HPNumber2X = Number(Moghunter.parameters['平移-最大生命数值 X'] || 140);
	Moghunter.scMenu_HPNumber2Y = Number(Moghunter.parameters['平移-最大生命数值 Y'] || 100);
	Moghunter.scMenu_MPNumberX = Number(Moghunter.parameters['平移-魔法数值 X'] || 100);
	Moghunter.scMenu_MPNumberY = Number(Moghunter.parameters['平移-魔法数值 Y'] || 124);
	Moghunter.scMenu_MPNumber2X = Number(Moghunter.parameters['平移-最大魔法数值 X'] || 140);
	Moghunter.scMenu_MPNumber2Y = Number(Moghunter.parameters['平移-最大魔法数值 Y'] || 151);
	Moghunter.scMenu_LVNumberX = Number(Moghunter.parameters['平移-等级数值 X'] || 95);
	Moghunter.scMenu_LVNumberY = Number(Moghunter.parameters['平移-等级数值 Y'] || 33);
	...
	Moghunter.menu_btn_list_length = 10;
...
ImageManager.loadMenusMain = function(filename) {
    return this.loadBitmap('img/menus_main/', filename, 0, true);
};
...
ImageManager.loadMenusMainCommands = function(filename) {
    return this.loadBitmap('img/menus_button/', filename, 0, true);
};		
...
Scene_Menu.prototype.loadBitmapsMain = function() {
	...
	for (var i = 0; i < $gameParty.members().length; i++) {
		 this._facesBitmaps[i] = ImageManager.loadMenusMainCommands(Moghunter.menu_btn_face_list[$gameParty.members()[i]._actorId] );
	};
	this._comBitmaps = []
...
MBustMenu.prototype.createCharaters = function() {
     this._char = new Sprite(ImageManager.loadPicture(Moghunter.menu_act_list[this._actor._actorId]));
