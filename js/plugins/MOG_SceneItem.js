...
/*:
 * @plugindesc (v1.1)[v1.2]  主菜单 - 全自定义道具界面
 ...
 * =============================================================================
 * 可完全自己定义的物品界面设置。
 * 配置有五个部分，物品类型、物品窗口、角色窗口、选择框、杂项数据。
 * 【现已支持插件关联资源的打包、加密】
 *
 * -----------------------------------------------------------------------------
 * ----关于物品类型
 * 这里的物品类型是固定的四种：道具、武器、防具、关键道具
 * 理论上你可以像主菜单那样通过特殊插件改变类型选项的数量，但是由于
 * 这里的选项在脚本编码上都被完全固定成4种了，所以无法确保与其它改变
 * 物品类型的插件之间的兼容性问题了。
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
 ...
 *
 * 资源-整体布局      (menus_item文件夹)
 * 资源-物品窗口      (menus_item文件夹)
 * 资源-道具选项      (menus_item文件夹)
 * 资源-武器选项      (menus_item文件夹)
 * 资源-防具选项      (menus_item文件夹)
 * 资源-关键道具选项  (menus_item文件夹)
 *
 * 资源-角色窗口      (menus_actorwindow文件夹)
 * 资源-选择框        (menus_actorwindow文件夹)
 * 资源-界限箭头      (menus_actorwindow文件夹)
 * 资源-选择全部角色  (menus_actorwindow文件夹)
 * 资源-生命条        (menus_actorwindow文件夹)
 * 资源-魔法条        (menus_actorwindow文件夹)
 *
...
ImageManager.loadMenusitem = function(filename) {
    return this.loadBitmap('img/menus_item/', filename, 0, true);
};
...
Scene_Item.prototype.loadBitmaps = function() {
	...
	this._comImg[1] = ImageManager.loadMenusitem(Moghunter.src_item_menu_com_4);
...
Scene_Item.prototype.createPartyArrow = function() {
	...
	this._partyArrowAni = [0,0];
	for (var i = 0; i < 2; i++) {
        this._partyArrow[i] = new Sprite(ImageManager.loadMenusActor(Moghunter.src_item_select_arrow));
...
Scene_Item.prototype.createAllMembersScope = function() {
     this._almSprite = new Sprite(ImageManager.loadMenusActor(Moghunter.src_item_select_all));
...
PartyWindowData.prototype.loadBitmaps = function() {
    this._layImg = ImageManager.loadMenusActor(Moghunter.src_item_select_Layout);
	this._hpMeterImg = ImageManager.loadMenusActor(Moghunter.src_item_select_hpmeter);
	this._mpMeterImg = ImageManager.loadMenusActor(Moghunter.src_item_select_mpmeter);
};
...
ActorDataWindow.prototype.createLayout = function() {
     this._layout = new Sprite(ImageManager.loadMenusActor(Moghunter.src_item_actor_Layout));
...
ActorDataWindow.prototype.refreshFaces = function() {
     this._faces.bitmap = ImageManager.loadMenusActor(Moghunter.menu_item_face_list[this._actor._actorId])
};
