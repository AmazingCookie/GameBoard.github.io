...
/*:
 * @plugindesc (v1.8)[v1.0]  敌人 - 技能动作 + 敌人呼吸效果
...
Game_Action.prototype.apply = function(target) {
	...
	if (target.isDead()) {this._motion_damage_duration = 0};	
	if (this.item() && this.item().damage.type != 1) {target._motion_damage_duration = 0};
};
...
//==============================
Spriteset_Battle.prototype.createBMotionShadow = function() {
	this._sprite_shadow = [];
	for (var i = 0; i < $gameTroop.members().length; i++) {
		 this._sprite_shadow[i] = new SpriteBattlerShadow();
		 this._sprite_shadow[i].z = 0;
		 this._battleField.addChild(this._sprite_shadow[i])
    };
};

//==============================
// * remove BMotion Shadow
//==============================
Spriteset_Battle.prototype.removeBMotionShadow = function() {
	for (var i = 0; i < this._sprite_shadow.length; i++) {
		 this._battleField.removeChild(this._sprite_shadow[i]);
    };
};
...
// * add BMotion Shadow
//==============================
Spriteset_Battle.prototype.addBMotionShadow = function() {
...
Sprite_Battler.prototype.set_breath_mode = function() {
		...
		if (Imported.MOG_EmergeMotion) {this.scale.y = 1.00};
};
...
Sprite_Battler.prototype.set_swing_mode = function() {
		...
		if (Imported.MOG_EmergeMotion) {this.rotation = 0};
};
...
Sprite_Battler.prototype.update_motion_idle = function() {
  if (Imported.MOG_EmergeMotion) {
	  if (this._battler && this._battler._emerge[0]) {
	      return;
	  };	
   };
